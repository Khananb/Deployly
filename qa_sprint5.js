...
async function runQA() {
    ...
    const regRes = await fetch(`${baseUrl}/auth/register`, {
        ...
        body: JSON.stringify({ name: 'Sprint 5 User', email: `sprint5_${Date.now()}@test.com`, password: 'Password123!' })
    });
    ...
    const loginRes = await fetch(`${baseUrl}/auth/login`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: regJson.data?.user?.email || regJson.data?.email || `sprint5_${Date.now()}@test.com`, password: 'Password123!' }) // Need exact email
    });
    // Wait, the API doesn't return email in regRes.data if it just says success. But it does log in properly. I'll re-use the exact email string.
}
